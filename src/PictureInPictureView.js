...
import Animated, { withDecay, withSpring, useAnimatedStyle, useAnimatedGestureHandler, useAnimatedReaction, useSharedValue, useDerivedValue, withTiming, runOnJS, Easing, } from 'react-native-reanimated';
...
function PictureInPictureView({ edgeConfig, initialX = 0, initialY = 0, deceleration = 0.985, minimumGlideVelocity = 120, scaleDuringDrag = 1.02, destroyOverlayColor = DEFAULT_OVERLAY_COLOR, onDestroy, style, children, }) {
    ...
    function calcOutOfBoundsWith(x, y) {
        'worklet';
        return {
    ...
    const outOfBounds = useDerivedValue(() => calcOutOfBoundsWith(translateX.value, translateY.value));
    /* Track whether the box is ready for drag-to-destroy */
    const releaseWillDestroy = useDerivedValue(() => {
        function canDestroyByDrag(edge) {
    ...
    const opacity = useDerivedValue(() => {
        ...
        const max = Math.max(0, edgeConfig.top.destroyByDrag ? top : 0, edgeConfig.bottom.destroyByDrag ? bottom : 0, edgeConfig.left.destroyByDrag ? left : 0, edgeConfig.right.destroyByDrag ? right : 0);
        return Math.min(fadeOpacity.value, 1 - max * 0.8);
    });
    /* Call onDestroy when opacity hits zero */
    useAnimatedReaction(() => opacity.value, (val, prev) => {
        if (destroying.value && val <= 0 && !!prev) {
            runOnJS(onDestroy)();
        }
    });
    /* Bounce back with spring when box is outside margins */
    useAnimatedReaction(() => !dragging.value && !destroying.value && outOfBounds.value, (oob) => {
        if (!oob)
            return;
        ...
        if (oob.top > -top.margin / boxHeight.value) {
            ...
            translateY.value = withSpring(viewHeight.value - boxHeight.value - bottom.margin, bottom.spring);
        }
        if (oob.left > -left.margin / boxWidth.value) {
            ...
            translateX.value = withSpring(viewWidth.value - boxWidth.value - right.margin, right.spring);
        }
    });
    const gestureHandler = useAnimatedGestureHandler({
        ...
        onActive: (event, ctx) => {
            ...
            if (resistTop && top > 0) {
                ...
                y = Math.min(y, y + bh * (1 - Math.pow(resistBottom, Math.min(bottom, 1)) - bottom));
            }
            if (resistLeft && left > 0) {
                x = Math.max(x, -bw * (1 - Math.pow(resistLeft, Math.min(left, 1))));
            }
            else if (resistRight && right > 0) {
                x = Math.min(x, x + bw * (1 - Math.pow(resistRight, Math.min(right, 1)) - right));
            }
            translateX.value = x;
            translateY.value = y;
        },
        onEnd: (event, _) => {
            ...
            function canDestroyByFling(edge, orientedAngle) {
            ...
            const destroyByFling = (vx < 0 && canDestroyByFling('left', angle)) ||
    ...
    const onLayoutBox = useCallback((evt) => {
        const { width, height } = evt.nativeEvent.layout;
        boxWidth.value = width;
        boxHeight.value = height;
    }, [boxWidth, boxHeight]);
    const onLayoutView = useCallback((evt) => {
        const { width, height } = evt.nativeEvent.layout;
        viewWidth.value = width;
        viewHeight.value = height;
    }, [viewWidth, viewHeight]);
    return (<View pointerEvents="box-none" style={styles.overlay} onLayout={onLayoutView}>
